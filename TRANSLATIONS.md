...
> **Quick start for contributors:** This guide helps you add a new language translation to Artifact OS in ~2 hours instead of ~8 hours. Follow the checklist, avoid common mistakes, and ship your PR with confidence.
...
- ✅ Alt text: `alt="Artifact OS banner"`
...
- ❌ Brand names: "Artifact OS", "Claude Code"
...
Artifact OS currently supports **19 languages** across different surfaces:
...
- ✅ Alt text in images: `alt="Artifact OS banner"` → `alt="Banner di Artifact OS"`
...
- ❌ Brand names: "Artifact OS", "Claude Code", "Anthropic", "Vercel"
...
- Use the English term with a brief explanation in parentheses on first use if no standard translation exists:
  ```
  Artifact OS è un'alternativa open-source (codice aperto) a Claude Design.
  ```
...
- Artifact OS, Claude Code, Claude Design
...
- `Skill` stays `Skill` when it names the Artifact OS/Claude skill format.
  Translate only generic prose such as "ability" or "capability" as
  `capacité`.
- `fork` stays `fork` when it names the Artifact OS conversation-fork feature
...
- `SOTA Harness` and `Harness` stay in English when they name the Artifact OS
...
**Example:**
```markdown
Artifact OS هو البديل مفتوح المصدر لـ Claude Design
```
...
```markdown
## Summary
Adds [Language] translation for Artifact OS documentation.
...
Every translation makes Artifact OS accessible to more developers worldwide.
...
**Ready to contribute?** Pick a language, follow the [Quick Start](#-quick-start-adding-your-language-in-5-steps), and submit your PR. We can't wait to see Artifact OS in your language! 🚀
