...
- Multiple Claude config directories cause Artifact OS's spawned Claude process to use different state than the user's terminal session.
...
- `401`, `apiKeySource: "none"`, missing/invalid token, or authentication failure: tell the user to run `claude`, use `/login`, then retry the same Artifact OS run.
...
- Windows credential or WSL/native mismatch indicators: suggest re-authenticating in the same shell/environment used by Artifact OS and checking Windows Credential Manager where applicable.
