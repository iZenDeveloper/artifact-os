...
### Step 2: Artifact OS Tailwind tokens
...
- `scripts/guard.ts` - added default Tailwind palette utility rejection for classes such as `text-red-500`, `bg-white`, `border-zinc-200`, `from-orange-500`, and related color utility namespaces so app UI uses Artifact OS token utilities.
...
- Style guard allowlist and promotion policy:
  ...
  | Repeated arbitrary app UI colors | Any unregistered hardcoded color after the second real app UI use | Promote to a named Artifact OS token before migration. One-off brand/user-content/illustration data keeps a reasoned allowlist entry. |
...
- `pnpm --filter @open-design/web build` - passed after adding the Artifact OS `@theme` token aliases and source variables.
