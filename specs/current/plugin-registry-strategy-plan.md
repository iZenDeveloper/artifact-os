# Artifact OS Plugin Registry Strategy Plan
...
Turn Artifact OS plugins from "installed local workflows plus a light marketplace index" into a registry-shaped ecosystem:

- Artifact OS hosts the official catalog on the main site and ships first-party plugins there.
- Community authors publish by opening GitHub PRs into the Artifact OS registry repository.
...
- `apps/daemon/src/plugins/publish.ts` now builds submission links for external catalogs and the Artifact OS registry target. Full GitHub fork/branch/PR mutation is still future backend work.
...
1. **Official web registry**
   - Hosted on the main Artifact OS site.
   - Renders official and community-approved plugins from the Artifact OS registry repo.
...
- **Artifact OS GitHub registry repo** is the v1 storage backend. It can later be swapped for a database backend without changing user-facing nouns.
...
```text
                 ...
                 public registry pages and docs
                               ...
                               v
+--------------------------------------------------+
| Artifact OS GitHub registry repo                 |
|                                                  |
...
- The file can initially live in the main Artifact OS repo if that lowers setup cost, but the product abstraction should still be `RegistryBackend`, not "read this monorepo path". Moving to `open-design/plugin-registry` later should be a data relocation, not a product rewrite.
...
- `od plugin login` wraps `gh auth login` with Artifact OS copy and required scopes/host guidance.
...
- "Contribute to Artifact OS registry" flow with required files, checklist, CLI command, and GitHub PR path.
...
Goal: make Artifact OS contributions feel like npm publish, while actually opening a GitHub PR.
...
- Should `official` marketplace trust be allowed for user-added URLs, or only for built-in source ids shipped by Artifact OS? Recommendation: only built-in sources can be `official`; user-added sources can be `trusted` or `restricted`.
...
The architecture is already pointed in the right direction: Artifact OS can become a "multi-source plugin registry" without giving up local-first/headless operation. The next move should not be a bigger gallery. It should be provenance, trust vocabulary, and exact resolution first; then the UI can safely graduate from "official plugins plus import" to "installed and available plugins across sources."
