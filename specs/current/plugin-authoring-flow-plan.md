...
- R2. The create entry navigates to Home, focuses `HomeHero`'s textarea, and pre-fills a prompt grounded in the Artifact OS plugin spec.
...
- No new plugin spec dialect. The generated folder must validate against the existing Artifact OS plugin shape in `docs/plugins-spec.md` and `docs/schemas/open-design.plugin.v1.json`.
...
- [x] U3. **Plugin authoring scenario**
  ...
  **Approach:**
  ...
  - The prompt should cite the Artifact OS plugin spec and require validation-ready output, not a prose-only explanation.
...
```text
Create an Artifact OS plugin for: <user goal>.
