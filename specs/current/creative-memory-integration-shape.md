...
contract between memory and the rest of Artifact OS: where signals come from,
...
- **Headless / CLI parity.** Per the dual-track rule, every signal capture
  ...
  external agents driving Artifact OS through `od` cannot contribute to the
...
overridable via `MEMORY_STORAGE_ROOT`. For Artifact OS integration, two
decisions:
