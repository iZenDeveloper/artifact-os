...
Define the target observability loop that lets Artifact OS improve agent
...
Artifact OS already reports completed runs to Langfuse and PostHog. The current
...
That means Artifact OS can inspect individual runs, but the system does not yet
...
The gap is therefore not "does Artifact OS know how to talk to object storage?"
...
Artifact OS should treat observability as a closed loop:
...
alerting surface. Artifact OS owns the domain model that maps a design-agent run
...
Artifact OS is a design product, so the input and output surface is heavier than
...
| --- | --- |
| Artifact OS object storage | Stores original user attachments, parsed text, thumbnails, OCR or embedding outputs, generated artifacts, preview screenshots, export bundles, and reproducibility files. |
...
- always emit an `attachment_manifest` and `artifact_manifest` with stable ids,
  ...
  redaction status, and Artifact OS storage references;
...
Artifact OS should add a registry layer above `ProjectStorage` instead of
...
apply, Artifact OS should not write a score value; it should record
...
- Define the Artifact OS object storage contract for attachments, parsed
...
- Store original heavy inputs and outputs in Artifact OS controlled storage,
