...
description: |
  ...
  Artifact OS browser-tab context, testing web apps, dogfooding Artifact OS
  previews, QA, bug hunts, or reviewing app quality. Prefer local Artifact OS
...
Use `agent-browser` for local Artifact OS preview validation: inspect rendered
...
For selected Artifact OS browser tabs and browser-use/browser-harness-style
...
5. If the selected Artifact OS context only provided a URL/title and no browser
...
Do not run Artifact OS's own daemon CLI as a browser automation tool. Commands
...
With the Artifact OS preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `Artifact OS`, current URL under `127.0.0.1:17573`,
visible Artifact OS UI text in the snapshot, and a screenshot at
