# Artifact OS — Nix flake

This flake exposes Artifact OS as a reproducible package, a `nix run` entry
...
Artifact OS's frontend is a static SPA that issues relative `/api/*`,
...
The static-server's environment does not need any Artifact OS env
