# Home Manager module for Artifact OS — primary interface for individual
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    ...
    (lib.mkIf (pkgs.stdenv.isLinux && cfg.autoStart) {
      systemd.user.services.open-design = {
        Unit = {
          Description = "Artifact OS daemon (user service)";
    ...
    (lib.mkIf (pkgs.stdenv.isLinux && cfg.webFrontend.enable) {
      systemd.user.services.open-design-web = {
        Unit = {
          Description = "Artifact OS web frontend (static file server)";
          After = ["network-online.target"];
          Wants = ["network-online.target"];
        };
        Install.WantedBy = ["default.target"];
