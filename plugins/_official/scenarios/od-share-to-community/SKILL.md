...
description: Package the user's just-finished work as an Artifact OS plugin without asking for fields the project files already answer, then surface the existing Add-to-My-plugins / Open-Design-PR buttons.
od:
...
Triggered by the post-completion "Share to Artifact OS" submission action. The user just finished a piece of work in this project and wants to ship it as a plugin. They have not been asked any questions yet.
...
- **Artifact OS PR** — opens a draft PR against `nexu-io/open-design` for the community catalog.
