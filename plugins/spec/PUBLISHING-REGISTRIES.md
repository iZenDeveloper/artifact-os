...
Artifact OS plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Artifact OS PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `open-design.json` as the Artifact OS sidecar.
...
2. Push a public GitHub repository or open an Artifact OS PR.
3. Add README install instructions for Artifact OS and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Artifact OS | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `open-design.json` | Open a PR to Artifact OS or publish a marketplace index entry pointing to the plugin repo. |
...
For Artifact OS plugin authors:
...
Use this path for normal Artifact OS plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Artifact OS PR.
