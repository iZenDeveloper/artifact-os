...
export async function buildWinLauncherPayloadArchive(
  ...
  const writeOverlay = async (input: { includeExecutable: boolean }): Promise<void> => {
    ...
    if (input.includeExecutable) {
      await cp(join(builtApp.unpackedRoot, "Artifact OS.exe"), join(overlayRoot, "payload", "Artifact OS.exe"));
    }
    await writeFile(
...
export async function validateWinLauncherPayloadArchive(input: {
  ...
  try {
    ...
    requirePayloadManifestValue(manifest.entry?.executable, "entry.executable", "payload/Artifact OS.exe");

    await stat(join(extractRoot, archiveRelativePath("payload/Artifact OS.exe")));
