...
async function runFixture(options: {
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "Artifact OS.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "Artifact OS.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "Artifact OS Helper.app"), { recursive: true });
  }
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "Artifact OS" } },
    });
  } catch (error) {
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/Artifact OS Helper\.app$/),
          expect.stringMatching(/Artifact OS\.app$/),
        ]),
      );
