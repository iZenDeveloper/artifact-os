...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Artifact OS-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Artifact OS-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Artifact OS Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Artifact OS Beta", "Artifact OS.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Artifact OS-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Artifact OS-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Artifact OS-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Artifact OS Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Artifact OS.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Artifact OS.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Artifact OS.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Artifact OS Beta.app",
      channel: "beta",
      entry: {
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Artifact OS Beta.app");
      expect(manifest.entry.executable).toBe("payload/Artifact OS Beta.app/Contents/MacOS/Artifact OS Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Artifact OS.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Artifact OS.exe"));
