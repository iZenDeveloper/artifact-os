...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Artifact OS-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Artifact OS-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Artifact OS"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Artifact OS", "Artifact OS.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Artifact OS-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Artifact OS-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Artifact OS-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Artifact OS", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Artifact OS.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Artifact OS.exe"))).resolves.toMatchObject({
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "Artifact OS",
      FileVersion: packagedVersion,
      ProductName: "Artifact OS",
      ProductVersion: "0.5.0.0",
    },
  );
