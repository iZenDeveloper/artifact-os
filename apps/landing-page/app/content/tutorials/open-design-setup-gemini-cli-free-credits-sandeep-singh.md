...
summary: 'Set up Artifact OS and drive it with Gemini CLI, so high-fidelity AI design generation runs on the Gemini free tier — an open-source, local-first workspace you can use without paying for the agent.'
date: 2026-05-11
...
This guide shows you how to run Artifact OS with **Gemini CLI** as the agent that drives generation — so prototypes, landing pages, and UI come out of one workspace while leaning on Gemini's generous free tier. It follows the path Sandeep Singh takes in [his hands-on walkthrough](https://www.youtube.com/watch?v=RqjrENimZP4), rewritten and updated to match the current release. Watch the video above for the live version, or read on.

The cost story is the point. Artifact OS itself is free and open source; the only thing you "pay" is the agent and model you bring. Bring Gemini CLI, authenticate with your Google account, and that bill effectively drops to zero.

![The Artifact OS workspace.](/tutorials/open-design-setup-gemini-cli-free-credits-sandeep-singh/01-workspace.webp)
*The Artifact OS workspace — open-source, local-first, and driven by whichever coding agent you connect.*

## What is Artifact OS?

Artifact OS is an open-source, local-first design platform that runs **on top of the coding agent you already use** rather than locking you to one model provider. It is "bring your own agent": Claude Code, Codex, Cursor, **Gemini**, GitHub Copilot, OpenCode, and 21+ others can each drive generation. You pick the agent; Artifact OS handles the design surface.
...
To use Artifact OS without ever opening the GUI — calling it as a skill or MCP server inside your agent — run:
...
This is the step that makes the workflow free. Artifact OS does not ship a model of its own — it drives whatever CLI you point it at — so you bring Gemini CLI and let its free tier do the work.
...
3. **Point Artifact OS at it.** Restart Artifact OS and open **Settings**. Gemini CLI now shows up as an available agent — select it, **Test the connection**, and you should get an `OK` back. Save, and you are ready to design.
...
Artifact OS keeps your work **project-scoped**: each project lives in its own folder, and switching folders switches projects. Inside a project you can create prototypes, live artifacts, slide decks, images, and even video and audio — not just UI.
...
2. **Write the brief.** Instead of one of the suggested prompts, describe what you want — Sandeep asks for a landing page. You can also attach a screenshot of a layout you like and ask Artifact OS to follow that theme.
...
- **You do not need a design system to start.** Begin from a built-in one, import a Claude Design project, or let Artifact OS infer defaults.

## FAQ

**Is it really free?**
Effectively, yes. Artifact OS is open source under Apache-2.0, so the software costs nothing. The only cost is the agent and model you bring — and with Gemini CLI authenticated via a Google account, you draw on a large daily free allowance. In the video, two prototypes plus iterations used roughly 2% of the day's quota, which refreshes daily.

**Which coding agents does it support?**
21+ agents, including Gemini, Claude Code, Codex, Cursor, GitHub Copilot, and OpenCode. Artifact OS detects the CLIs already installed on your machine and lets you pick one as the default.
...
No. Artifact OS ships branded design systems and templates as starting points, and you can import an existing design system or let it infer sensible defaults.
