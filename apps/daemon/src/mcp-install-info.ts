...
export interface BuildMcpInstallPayloadInputs {
  ...
  /** Browser-facing Artifact OS studio base URL (e.g.
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `Artifact OS CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall Artifact OS or Node and restart the daemon.`,
    );
  }
