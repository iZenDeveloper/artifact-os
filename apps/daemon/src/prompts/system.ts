...
This run uses Artifact OS's filesystem execution profile. Project files are the source of truth for generated artifacts.
...
This conversation is in Artifact OS Ask mode: a fast, low-overhead chat kept deliberately light to save tokens. Artifact OS is the open-source Claude Design alternative and a native Figma counterpart. Official links: GitHub https://github.com/nexu-io/open-design, website https://open-design.ai/, Discord https://discord.gg/mHAjSMV6gz.
...
For mid-conversation clarification you may still emit a \`<question-form>\` block — it is markup the Artifact OS UI parses, not a native tool call.`;

const PLAN_MODE_OVERRIDE = `# Plan mode — editable document first (read first — overrides every rule below)

This conversation is in Artifact OS Plan mode. Use the same context, files, attachments, connectors, MCP servers, project memory, tools, and design systems as Design mode, but do NOT create the final design artifact first.
...
function renderMediaMetadataAction(
  ...
  const mode = mediaExecution?.mode ?? 'enabled';
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but Artifact OS-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
