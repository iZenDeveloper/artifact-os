...
export function createAgentRuntimeToolPrompt(
  ...
  return [
    ...
    '- `OD_BIN` is the absolute path to the Artifact OS CLI script. On POSIX shells run wrappers with `"$OD_NODE_BIN" "$OD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
...
export async function startServer({
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // Artifact OS relays them to the agent so the model can call those tools.
  ...
  orbitService.setRunHandler(async ({
    ...
    design.runs.start(run, () => startChatRun({
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside Artifact OS.',
