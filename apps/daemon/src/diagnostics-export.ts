...
export interface DiagnosticsHandlerOptions {
  ...
  runsDir?: string | null;
  /** Artifact OS data dir (OD_DATA_DIR), used to locate the AMR OpenCode home. */
  dataDir?: string | null;
}
  ...
  return async (_req, res) => {
    try {
      ...
      const result = await buildDiagnosticsZip({
        ...
        redaction: { username },
        crashReports: {
          // Restrict to Artifact OS's own process names. A generic "Electron"
          ...
          matchSubstrings: ['Artifact OS', 'open-design'],
          withinDays: 7,
          maxReports: 10,
          homeDir: home,
        },
