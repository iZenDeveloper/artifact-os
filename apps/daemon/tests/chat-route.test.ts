...
process.stdin.on('end', () => {
  ...
  console.log(JSON.stringify({ type: 'step_start' }));
  console.log(JSON.stringify({ type: 'text', part: { text: '我来帮你创建一个通用的 Artifact OS 插件脚手架。先读取文档规范，再生成插件文件。' } }));
  console.log(JSON.stringify({ type: 'step_finish', part: { tokens: { input: 1, output: 1 } } }));
  process.exit(0);
});
`,
      async () => {
        const createResponse = await fetch(`${baseUrl}/api/runs`, {
          ...
          body: JSON.stringify({
            ...
            message: '请创建一个可刷新、可审计、由 API 驱动的 Artifact OS 插件脚手架。',
          }),
        });
...
`,
      async () => {
        const response = await fetch(`${baseUrl}/api/chat`, {
          ...
          body: JSON.stringify({
            agentId: 'opencode',
            message: 'build the Artifact OS landing page',
