...
describe('browser use diagnostics', () => {
  it('detects Browser Use prompts from the Artifact OS Browser menu', () => {
    expect(isBrowserUseRequested('hello')).toBe(false);
    expect(isBrowserUseRequested('@agent-browser\n\nBrowser tab context:')).toBe(true);
    expect(isBrowserUseRequested('Use the selected Artifact OS Browser tab as the bound target.')).toBe(true);
  });
