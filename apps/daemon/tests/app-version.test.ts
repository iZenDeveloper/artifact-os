...
describe('app version helpers', () => {
  ...
  it('prefers packaged app version metadata from the environment', () => {
    expect(resolveAppVersionInfo({
      ...
      resourcesPath: '/Applications/Artifact OS.app/Contents/Resources',
      execPath: '/Applications/Artifact OS.app/Contents/Resources/open-design/bin/node',
      platform: 'darwin',
      arch: 'arm64',
    })).toEqual({
  ...
  it('detects packaged runtimes without sidecar protocol knowledge', () => {
    expect(isPackagedRuntime({ resourcesPath: '/Applications/Artifact OS.app/Contents/Resources' })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/Applications/Artifact OS.app/Contents/Resources/open-design/bin/node',
      platform: 'darwin',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Artifact OS\\resources\\open-design\\bin\\node.exe',
      platform: 'win32',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/opt/Artifact OS/resources/open-design/bin/node',
      platform: 'linux',
    })).toBe(true);
