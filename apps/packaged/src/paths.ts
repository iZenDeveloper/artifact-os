  ...
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "Artifact OS's packaged runtime requires OD_DATA_DIR to be an absolute path.",
          "",
          `Configured value: ${odDataDir}`,
          "",
          "Set OD_DATA_DIR to an absolute path (for example, C:\\\\Users\\\\You\\\\OpenDesign on Windows or /Users/you/OpenDesign on macOS/Linux) and relaunch Artifact OS.",
        ].join("\n"),
        { title: "Artifact OS cannot start with this OD_DATA_DIR" },
      );
    }
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "Artifact OS's packaged runtime requires OD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
...
export function resolvePackagedNamespacePaths(
  ...
  // `~/Library/Application Support/Artifact OS Prerelease/` on mac. Custom
