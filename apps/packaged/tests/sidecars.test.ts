...
describe('resolvePackagedElectronNodeCommand', () => {
  it('uses the hidden Electron helper as the macOS Electron-as-Node command when available', async () => {
    ...
    try {
      const appPath = posix.join(root.replaceAll('\\', '/'), 'Artifact OS.app');
      const execPath = posix.join(appPath, 'Contents', 'MacOS', 'Artifact OS');
      const helperPath = posix.join(
        appPath,
        'Contents',
        'Frameworks',
        'Artifact OS Helper.app',
        'Contents',
        'MacOS',
        'Artifact OS Helper',
      );
  ...
  it('falls back to the main executable when the macOS helper is unavailable', async () => {
    ...
    try {
      const execPath = join(root, 'Artifact OS.app', 'Contents', 'MacOS', 'Artifact OS');
  ...
  it('keeps the main executable on non-macOS platforms', async () => {
    const execPath = '/opt/Artifact OS/open-design';
