...
const ISSUE_4638_LOG = `Error [ERR_MODULE_NOT_FOUND]: Cannot find package 'better-sqlite3' imported from /Applications/Artifact OS.app/Contents/Resources/app/prebundled/daemon/chunks/server-PULTSXNL.mjs
...
const DAEMON_EXIT_MESSAGE =
  'daemon exited before reporting status (code=1, signal=none); see /Users/liudetao/Library/Application Support/Artifact OS/namespaces/release-stable/logs/daemon/latest.log for details';
const WEB_EXIT_MESSAGE =
  'daemon exited before reporting status (code=1, signal=none); see /Users/liudetao/Library/Application Support/Artifact OS/namespaces/release-stable/logs/web/latest.log for details';
...
describe('classifyStartupFailure', () => {
  ...
  it('classifies a Windows (backslash) web-start log path as web-start', () => {
    ...
    const winWebMessage =
      'daemon exited before reporting status (code=1, signal=none); see C:\\Users\\Alice\\AppData\\Roaming\\Artifact OS\\namespaces\\release-stable\\logs\\web\\latest.log for details';
...
describe('scrubUserPaths', () => {
  it('redacts the user home directory but keeps the rest of the path', () => {
    const scrubbed = scrubUserPaths(
      '/Users/liudetao/Library/Application Support/Artifact OS/namespaces/release-stable/logs/daemon/latest.log',
    );
