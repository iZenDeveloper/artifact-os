...
describe("resolvePackagedLauncherRuntime", () => {
  ...
  it("uses the active launcher payload when runtime state and payload manifest are valid", async () => {
    ...
    try {
      ...
      const resourcesPath = join(versionPaths.payloadRoot, "Artifact OS Beta.app", "Contents", "Resources");
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          channel: "beta",
          launchPath: "/Applications/Artifact OS Beta.app",
          namespace: config.namespace,
          schemaVersion: LAUNCHER_SCHEMA_VERSION,
        })}\n`,
      );
      ...
      expect(runtime.electronNodeCommand).toBeNull();
      expect(runtime.installedLaunchPath).toBe("/Applications/Artifact OS Beta.app");
  ...
  it("uses the Windows payload executable as the Electron-as-Node command when payload Node is absent", async () => {
    ...
    try {
      ...
      const payloadExePath = join(versionPaths.versionRoot, "payload", "Artifact OS.exe");
