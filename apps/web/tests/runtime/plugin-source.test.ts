...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the Artifact OS repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'Artifact OS', url: 'https://github.com/nexu-io' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('Artifact OS');
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('Artifact OS')).toBe('AO');
