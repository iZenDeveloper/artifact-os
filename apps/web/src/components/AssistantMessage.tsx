...
function SkillPluginCandidateCard({
  ...
  async function share(action: "contribute-open-design") {
    ...
    try {
      ...
      setNotice({
        message: `Artifact OS contribution task started for ${data?.path ?? "the draft"}.`,
      });
    } catch (err) {
      setNotice({ message: err instanceof Error ? err.message : String(err) });
    } finally {
...
interface Props {
  ...
  // Click handler for the post-completion "Share to Artifact OS" submission
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|Artifact OS PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
