...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  queueDrain?: boolean;
  /** The Artifact OS Cloud balance gate already ran for this exact send at
...
function buildCreateDesignSystemFromProjectPrompt(input: {
  ...
  return [
    'Create this project as a complete Artifact OS design system workspace.',
    '',
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'Artifact OS selected or inferred these workspace contexts for this turn. Treat absolute paths as reference context unless the user explicitly asks to edit them.',
...
/** Set by the home create flow when its submit already ran the Artifact OS
...
export function ProjectView({
    ...
    (
      ...
      setMessages((curr) => {
        ...
        const next = curr.map((m) => {
          ...
          // Bail when the updater returns the same reference — critical for
          // high-frequency SSE callbacks (onRunEventId) that would otherwise
          // allocate a new messages array every event and trip
          // "Maximum update depth exceeded" under React's nested-update guard.
  ...
  useEffect(() => {
    ...
    const attachRecoverableRuns = async () => {
      ...
      for (const message of messages) {
        ...
        // Throttle lastRunEventId bookmarks — every SSE frame can carry a new
        // id; applying each via setMessages (+ prior textBuffer.flush) nested
        // updates past React's max depth during reattach replay.
        const reattachEventIdBinder = createRunEventIdBinder({
        ...
        void reattachDaemonRun({
          ...
          handlers: {
            ...
            onDone: async () => {
              ...
              if (runMayFinalize) {
                textBuffer.flush();
                reattachEventIdBinder.flush();
              }
              textBuffer.cancel();
              reattachEventIdBinder.cancel();
          ...
          onRunEventId: reattachEventIdBinder.onRunEventId,
        })
          .catch((err) => {
  ...
  const handleSend = useCallback(
    async (
      ...
      // Artifact OS Cloud pre-run balance gate: a definitively insufficient
      ...
      // Throttle SSE event-id bookmarks (see createRunEventIdBinder).
      const sendEventIdBinder = createRunEventIdBinder({
        apply: (lastRunEventId) => {
      ...
      const handlers = {
        ...
        onDone: (fullText = '') => {
          ...
          if (!runMayFinalize) {
            textBuffer.cancel();
            sendEventIdBinder.cancel();
            cancelSendTextBuffer();
            clearTraceTouchedFilePaths();
            return;
          }
          textBuffer.flush();
          sendEventIdBinder.flush();
          textBuffer.cancel();
          sendEventIdBinder.cancel();
          cancelSendTextBuffer();
          for (const ev of parser.flush()) {
      ...
      if (config.mode === 'daemon') {
        ...
        void streamViaDaemon({
          ...
          onRunStatus: (runStatus) => {
            ...
            const runMayFinalize =
              !supersededRunsRef.current.has(controller);
            if (isTerminalRunStatus(runStatus)) sendEventIdBinder.flush();
            updateMessageById(
              assistantId,
              (prev) => ({
                ...prev,
          ...
          onRunEventId: sendEventIdBinder.onRunEventId,
        });
        return true;
      } else {
  ...
  // "Share to Artifact OS" — kicks off the bundled `od-share-to-community`
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'Artifact OS PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the Artifact OS fork exists',
...
const DESIGN_RESULT_DELIVERY_FAILED_DETAIL =
  'The design result was generated, but Artifact OS could not save it to the project.';
...
/**
 * Coalesce high-frequency SSE `id:` bookmarks into throttled React state
 * updates. Every daemon event can carry a new id; applying each one via
 * setMessages (and often textBuffer.flush) was nesting dozens of updates per
 * frame and surfacing "Maximum update depth exceeded" from onRunEventId.
 */
export function createRunEventIdBinder(options: {
  apply: (eventId: string) => void;
  /** Default 200ms — short enough for resume bookmarks, long enough to batch. */
  throttleMs?: number;
}): { onRunEventId: (eventId: string) => void; flush: () => void; cancel: () => void } {
  ...
  const flush = () => {
    ...
    if (latest === null || latest === applied) return;
    applied = latest;
    options.apply(latest);
  };

  return {
    onRunEventId: (eventId: string) => {
      if (!eventId || eventId === latest) return;
      latest = eventId;
      if (timer !== null) return;
      timer = setTimeout(() => {
        timer = null;
