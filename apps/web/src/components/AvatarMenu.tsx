...
function displayAgentName(agent: Pick<AgentInfo, 'id' | 'name'>): string {
  return agent.id === 'amr' ? 'Artifact OS' : agent.name;
}
...
export function AvatarMenu({
  ...
  // whenever the Artifact OS runtime is installed — so the Artifact OS agent row
          ...
          {config.mode === 'daemon' && installedAgents.length > 0 ? (
            <>
              ...
              {installedAgents.map((a) => {
                ...
                // Artifact OS row carries the account (balance + plan) inline,
