...
interface Props {
  ...
  onBack?: () => void;
  backLabel?: string;
  /** Collapses the chat pane (workspace focus mode). When provided, the
      header's leading button becomes this toggle instead of the back nav —
      re-expanding happens from the workspace's own show-chat button. */
  onCollapse?: () => void;
                  ...
                  {errorDiagnosticText ? (
                    <div className={`run-error__source${errorSourceOpen ? ' is-open' : ''}`}>
                      <div className="run-error__source-head">
                        ...
                        >
                          <Icon name={copiedErrorDiagnostic ? 'check' : 'copy'} size={14} />
                        </button>
                      </div>
...
function MessageSessionModeChip({
  ...
  return (
    <div
      ...
      title={label}
    >
      <Icon name={mode === 'chat' ? 'comment' : 'sparkles'} size={14} />
      <span>{label}</span>
    </div>
  );
}
