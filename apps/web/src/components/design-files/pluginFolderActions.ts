...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Artifact OS community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Artifact OS CLI workflow from the current project workspace:',
    '',
...
// Artifact OS issue URL and belongs to the "Artifact OS PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an Artifact OS issue URL and belongs to the "Artifact OS PR" button.',
    '',
    'Run this deterministic Artifact OS CLI workflow from the current project workspace:',
    '',
