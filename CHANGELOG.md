...
- 📦 **Trying Artifact OS gets easier on Windows and Linux.** Windows gets a portable zip path; Linux gets Docker/Podman Compose one-click setup. Less install friction, faster first run. (#2937, #2414) Thanks @PerishCode, @epicsagas.
- 🔌 **MCP clients can now do real workspace work.** Write files, delete files, delete projects, resolve the active project directory, run generation loops, and bootstrap Codex from one place. External clients can now participate in the Artifact OS workspace instead of only observing it. (#2416, #2802, #3141) Thanks @YOMXXX, @papperrollinggery, @lefarcen.
- 🛂 **Network changes stop requiring a restart.** Switch the system proxy while Artifact OS is running; the next outbound request picks it up automatically. (#3093) Thanks @mrcfps.
...
#### 🤖 Agents & runtimes
- **Artifact OS AMR (vela) as a first-class ACP stdio agent**, with onboarding, sign-in pill, and bundled `vela` CLI inside the installer. (#2355) Thanks @lefarcen.
...
- **Refresh Artifact OS app visuals** — new app icons, logo, brand glyphs. ([#2436])
...
- Pack/win: close detection gaps that let `Artifact OS.exe` stay locked at install time. ([#823])
...
- Release: Artifact OS 0.5.0 changelog landing. ([#820])
...
A multi-protocol leap: Artifact OS now ships as an MCP server, ships Critique Theater (Design Jury) Phase 4, gains live-reload + Tweaks mode + live artifacts in the preview pane, and adds five new agent / runtime adapters. 71 merged PRs from 40+ contributors over two days. Linux AppImage packaging landed in tooling, but the stable Linux artifact is deferred from 0.4.0 while containerized release packaging is hardened.

### Added

#### MCP & agent integration
- **`od mcp` — expose Artifact OS as a stdio MCP server.** Coding agents in other repos (Claude Code, Codex, Cursor, VS Code, Antigravity, Zed, Windsurf) can read files from local Artifact OS projects directly, including the project the user has open in the Artifact OS app right now. ([#399])
...
First public release of Artifact OS — a local-first, open-source alternative to Anthropic's Claude Design. It detects your installed code-agent CLI, runs design skills against curated design systems, and streams artifacts into a sandboxed in-app preview.
...
- Initial project structure, project rename "Open Claude Design" → "Artifact OS", naming optimization. ([#1], [#2])
