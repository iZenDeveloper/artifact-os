...
This note describes how an external service can use Artifact OS as a creative
runtime while keeping media provider governance outside Artifact OS.

Artifact OS contributes project context, skills, design systems, previews,
...
Run-scoped `mediaExecution` controls only Artifact OS-owned media generation.
It applies to:

- token-gated `/api/tools/media/generate`
- in-run `od media generate` when `OD_TOOL_TOKEN` is present
- Artifact OS's Codex image generation prompt override
- Artifact OS's media generation prompt contract
...
Artifact OS should not grow a generic provider router, provider account pool,
...
Use HTTP/SSE between the external service and Artifact OS. Avoid shelling out to
...
2. The external service creates or selects an Artifact OS project.
3. The external service starts a run with `mediaExecution` set to the desired
   Artifact OS-owned media policy.
4. The run includes skills and MCP tools that describe the external media
   workflow.
5. Artifact OS handles design/runtime work. External MCP media tools handle
...
For runs where Artifact OS may use only a narrow part of its own media path,
use allowlists:
...
Use Artifact OS project context, design-system guidance, and artifact previews
to plan the campaign. When media bytes are needed, use the configured external
media MCP tools. Do not call Artifact OS-owned media generation unless the run
...
without making Artifact OS own provider auth or budgets.
...
the external service, not to Artifact OS core.

The MCP server should receive its own credentials from the external service or
from its deployment environment. Artifact OS should only see the MCP tool
...
- The external service uploads the fulfilled asset through an Artifact OS
...
Artifact OS project files. Project artifacts should describe creative output,
...
still available for normal Artifact OS media generation outside the in-run tool
...
- `request-only` run mode in Artifact OS core
- Artifact OS media request persistence for external provider work
- a generic HTTP executor provider inside Artifact OS
- provider credentials or account pools in Artifact OS
- Artifact OS-owned provider budgets, retries, or global media limits
