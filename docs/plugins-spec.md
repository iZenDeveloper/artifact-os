# Artifact OS Plugin & Marketplace Spec (v1)

> **In one sentence:** Artifact OS plugins turn portable `SKILL.md` capabilities into marketplace-ready, one-click design workflows while preserving compatibility with existing agent skill catalogs, headless CLI use, and self-hosted deployment.
...
A **Plugin** is the unit of distribution for Artifact OS. Where a [Skill](skills-protocol.md) describes a single capability that an agent can run, a Plugin is the shippable bundle around it: one or more skills, an optional design system reference, optional craft rules, optional Claude-plugin assets, a preview, a use-case query, an asset folder, and a small machine-readable sidecar that powers OD's marketplace surface. A plugin is always anchored to a portable `SKILL.md` so it is publishable to every existing skill catalog without modification.
...
| Question | Figma-era plugin assumption | Artifact OS v1 answer |
| --- | --- | --- |
...
**Current implementation clarification:** `discovery -> plan -> generate -> critique` is a reference pipeline shape, not a fixed hard-coded wizard. A plugin snapshot can carry `od.pipeline.stages[].atoms[]`; the daemon resolves that snapshot, injects the active plugin block plus active stage atom blocks into the system prompt, emits stage events, and lets the agent work through the pipeline. When the user has not explicitly selected a plugin, OD still does **not** launch a generic naked agent: the base Artifact OS designer prompt and discovery rules are always present. Product entry points bind sensible defaults on top of that base: Home free-form input routes through the bundled, hidden `od-default` scenario, while typed New Project flows choose the default bundled scenario for the project kind. `od-default` is a router and task-shaper; it should guide the run into the normal design pipeline, not be treated as a standalone "make it beautiful" aesthetic engine.
...
19. [Why this is a meaningful step for Artifact OS](#19-why-this-is-a-meaningful-step-for-open-design)
...
6. The OD core engine, atomic capabilities, and plugin runtime are all reachable from CLI so any code agent can drive Artifact OS headlessly.
...
```json
{
  ...
  "author":   { "name": "Artifact OS", "url": "https://open-design.ai" },
  "license":  "MIT",
...
- `specVersion` — the Artifact OS plugin spec version used to interpret the manifest. This is distinct from plugin `version` and is frozen into apply snapshots for replay.
...
```json
{
  ...
  "version": "1.0.0",
  "owner":    { "name": "Artifact OS", "url": "https://open-design.ai" },
...
A code agent driving Artifact OS through the CLI typically does:
...
The desktop app registers the `od://` URL scheme; clicking a button on `open-design.ai/marketplace` either launches the desktop or, if it is not installed, falls back to a "How to install Artifact OS" flow.
...
## 19. Why this is a meaningful step for Artifact OS
