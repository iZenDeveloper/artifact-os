...
export function buildSocialSharePayload(input: SocialShareRequest): SocialShareResponse {
  ...
  const fallbackTitle = kind === 'project-html' ? 'Artifact OS project' : 'Artifact OS';
  const title = cleanText(input.title, fallbackTitle);
  const fallbackText = kind === 'project-html'
    ? `Built with Artifact OS: ${title}. Artifact OS repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`
    : 'Artifact OS is an open-source workspace for creating, editing, deploying, and handing off design artifacts.';
